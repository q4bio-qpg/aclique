...
#    print "2: @mergeable\n";
	...
	# NB: this is equivalent to simply *not* adding to NEAR if any sibling
	# has a long branch.  Ie we could just change nodes_within_dist
	foreach (@$far) {
	    print "FAR: $_ ",$parent->{$_},"\n";
	    my $p = $parent->{$_};
	    print "SIBLINGS: $tree->{$p}\n";

	    # Look for a sibling in NEAR
	    my $sib = 99999;
	    my $s;
	    foreach my $s_ (split(",",$tree->{$p})) {
		$s_ =~ m/(^[^:]*)(?::(.*))?/;
		for ($sib=0; $sib < scalar(@$near); $sib++) {
		    last if ($near->[$sib] eq $s_);
		}
		if ($sib < scalar(@$near)) {
		    $s= $s_;
		    last;
	    ...
	    # Find that sibling in our new near_node
	    my $ni;
	    my @nc = split(",", $tree->{$near_node});
	    my $c;
	    for ($ni=0; $ni < scalar(@nc); $ni++) {
		$c = $nc[$ni];
		$c =~ m/(^[^:]*)(?::(.*))?/;
		last if $c eq $s;
	    }
	    ...
	    # We've now found our sibling, so replace it with a new tree
	    # of that plus us.
	    print "Replace near_node[$ni] with ($s, $_)\n";
	    my $mp = "F/$merge_num";
	    $merge_num++;
	    #$tree->{$mp} = "$s,$_:$dist->{$_}";
	    $tree->{$mp} = "$s,$_";
	    ...
	    #$nc[$ni] = "$mp:$dist->{$s};
	    $nc[$ni] = $mp;
	    print "Replace $near_node from $tree->{$near_node} with ",
		join(",",@nc)," where $mp=$tree->{$mp}\n";
	    $tree->{$near_node} = join(",",@nc);
	    
	    # remove far node from it's old parent
	    $ni = 0;
	    @nc = split(",", $tree->{$p});
	    for ($ni=0; $ni < scalar(@nc); $ni++) {
		my $c = $nc[$ni];
	    ...
	    splice(@nc, $ni, 1);
	    print "Replace $p from $tree->{$p} with ",join(",", @nc),"\n";
    ...
    print "Node tree->{$node} = $tree->{$node}\n";
